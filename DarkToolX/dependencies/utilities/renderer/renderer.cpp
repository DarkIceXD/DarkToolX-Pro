...
void render::text(std::int32_t x, std::int32_t y, unsigned long font, const std::string& text, bool centered, color color) {
	wchar_t temp[128];
	int text_width, text_height;
...
void render::draw_circle(std::int32_t x, std::int32_t y, std::int32_t radius, std::int32_t segments, color color) {
	float step = math::M_PI * 2 / segments;
	for (float a = 0; a < (math::M_PI * 2.0); a += step) {

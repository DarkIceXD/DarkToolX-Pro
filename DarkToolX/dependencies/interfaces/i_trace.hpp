...
public:
	...
	explicit VectorAligned(const vec3_t& v)
	{
...
struct ray_t {
	...
	ray_t(const vec3_t& vecStart, const vec3_t& vecEnd, const vec3_t& min, const vec3_t& max) noexcept :
		m_start(vecStart), m_delta(vecEnd - vecStart), m_extents(max - min), m_world_axis_transform(nullptr), m_is_ray(false)
	{
		this->m_is_swept = (this->m_delta.length_sqr() != 0.f);
	}

	ray_t(const vec3_t& vecStart, const vec3_t& vecEnd) noexcept :
		m_start(vecStart), m_delta(vecEnd - vecStart), m_world_axis_transform(nullptr), m_is_ray(true)
	{
		this->m_is_swept = (this->m_delta.length_sqr() != 0.f);
	}
};
...
public:
	constexpr trace_filter(void* skip) : skip(skip) {}

...
void features::slow_walk(c_usercmd* cmd, float& forwardmove, float& sidemove)
{
	...
	const auto max_speed = csgo::local_player->is_scoped() ? weapon_data->weapon_max_speed_alt : weapon_data->weapon_max_speed;
	if (csgo::local_player->velocity().length() > (max_speed * 0.34))
	{
		cmd->buttons |= in_walk;

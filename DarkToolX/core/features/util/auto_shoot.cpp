...
void features::util::auto_shoot(c_usercmd* cmd, weapon_t* weapon, const weapon_info_t* weapon_data)
{
	...
	const auto entity = auto_wall(csgo::local_player->get_eye_pos(), math::angle_vector(cmd->viewangles + csgo::local_player->recoil()), weapon_data, csgo::cfg.aimbot().auto_wall).entity;
	if (!entity)
		return;
